...
print('{} {}'.format(max_ship[0][0],round(max_ship[0][1],2)))
print('{} {}'.format(max_ship[0][0],round(max_ship[0][1],2)), file=results)
...
for row in purchase:
    if row[1] in profit:
        profit[row[1]] += product_price[row[1]]*row[4]-row[-1]
    else:
        profit[row[1]] = product_price[row[1]]*row[4]-row[-1]
...
# Sort from highest to lowest
max_read_profit = sorted(sorted(hover_profit.items()), key=lambda x: x[1], reverse=True)
